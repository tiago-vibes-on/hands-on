# Hands-on repository instructions

- Use English for source code, documentation, comments, and generated content.
- Each project follows the `<project>/initial` and `<project>/final` layout.
- The `initial` and `final` applications must be independently runnable.
- Keep projects independent. Do not modify a sibling project unless the task explicitly requires it.
- Read the nearest `SPEC.md` before making a change.
- Run checks relevant to a change and report the result.
