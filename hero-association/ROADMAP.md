...
- [ ] Add quest definitions with objectives, creature groups, duration,
  rewards, difficulty, and recommended classes.
- [ ] Add a command to start a quest with a party. Validate hero availability,
  party membership, and quest eligibility.
- [ ] Persist quest status and timestamps: available, in progress, completed,
  failed, and cancelled if cancellation is allowed.
- [ ] Add a command or scheduled process that advances an in-progress quest.
- [ ] Replace the frontend's fixed active quest and available quest cards with
  API data.

## Milestone 4 — Server-side automatic combat

- [ ] Define the initial combat formulas for basic attack damage, target
  selection, attack timers, armor, attack speed, critical chance, and critical
  damage.
- [ ] Implement a deterministic combat engine with injectable time and random
  values so it can be tested reliably.
- [ ] Resolve individual hero and creature timers, health, mana, deaths, and
  the initial mage spells on the server.
...
- [ ] Define combat formulas and initial creature attributes.
